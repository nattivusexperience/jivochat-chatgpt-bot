...
def detect_lang(text: str) -> str:
    t = (text or '').lower()
    import re as _re
    t_clean = _re.sub(r'[^a-záéíóúñü¿?\s]', ' ', t)
    en_markers = [' how ', 'how much', 'price', 'prices', 'cost', 'what', 'whats', 'when', 'where', 'is there', 'do you', 'include', 'included', 'children', 'cancellation', 'refund', 'available']
    if any(m in (' ' + t_clean + ' ') for m in en_markers):
        return 'en'
    es_markers = [' qué', 'que ', 'cuál', 'cual', 'cuanto', 'cuánto', 'precio', 'precios', 'fechas', 'incluye', 'dónde', 'donde', 'cuando', 'política', 'cancelación', 'disponibilidad']
    if any(m in (' ' + t_clean + ' ') for m in es_markers) or '¿' in t or '¡' in t:
        return 'es'
    words = [w for w in _re.split(r'\s+', t) if w]
    en_guess = sum(1 for w in words if _re.fullmatch(r'[a-z]+\??', w)) >= max(1, int(0.6 * len(words)))
